...
import com.example.samrajyalakshmitemple.databinding.FragmentInformationCentresBinding
...
class InformationCentresFragment : BaseFragment<FragmentInformationCentresBinding>() {
    override val bindingInflater: (LayoutInflater, ViewGroup?, Boolean) -> FragmentInformationCentresBinding =
        FragmentInformationCentresBinding::inflate


    private val informationCentresViewModel by viewModels<PoojaSevaViewModel>()
    override fun setup() {
        val view5 = requireActivity().findViewById<MaterialButton>(R.id.login_signout_btn)
        view5?.setOnClickListener {
            if (!savedPrefManager.isLogin()){
                Navigation.findNavController(requireActivity(), R.id.nav_host_fragment)
                    .navigate(R.id.action_info_to_login)
            }else{
        ...
        informationCentresViewModel.setViewPagerList()


        informationCentresViewModel.viewPagerLive1.observe(viewLifecycleOwner) {
            val itemsPagerAdapter =
                informationCentresViewModel.viewPagerLive1.value?.let { it1 ->
